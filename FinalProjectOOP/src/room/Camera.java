...
import space3D.Rectangle;
import space3D.Triangle;
...
// Lay 4 dinh cua vung nhin thay
	...
	public class Image {
		...
		public Image() {
			this.widthImg = (int) ((Math.tan(angleWidth / 2)
					* Math.sqrt(3e5 / (Math.tan(angleWidth / 2) * Math.tan(angleHeight / 2)))));

			this.heightImg = (int) ((Math.tan(angleHeight / 2)
					* Math.sqrt(3e5 / (Math.tan(angleWidth / 2) * Math.tan(angleHeight / 2)))));
		...
		public void setMatrixPoint() {
			...
			if (Calculate3D.scalar(oppositePlane.getN(), new Vector3D(1, 0, 0)) == 0) {
				if ((projectionInOp.y - camPosition.y) < 0)
					scale = -scale;
				for (int i = 0; i < widthImg; i++) {
					for (int j = 0; j < heightImg; j++) {
						matrixPoint[i][j] = new Point(x + (i - widthImg / 2) * scale, y,
								z + (j - heightImg / 2) * Math.abs(scale));
					}
				}
			} else {
				if ((projectionInOp.x - camPosition.x) > 0)
					scale = -scale;
				for (int i = 0; i < widthImg; i++) {
					for (int j = 0; j < heightImg; j++) {
						matrixPoint[i][j] = new Point(x, y + (i - widthImg / 2) * scale,
								z + (j - heightImg / 2) * Math.abs(scale));
					}
				}
			}
